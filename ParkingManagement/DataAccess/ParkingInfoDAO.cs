...
namespace DataAccess
{
    public class ParkingInfoDAO
    {
        ...
        public async Task<IEnumerable<ParkingInfo>> GetParkingInfos(string status)
        {
            ...
            try
            {
                ...
                if (status.Equals("All"))
                {
                    ...
                    .Include(p => p.Pricing)
                    .Include(p => p.Slot)
                    .Include(p => p.Vehicle).OrderByDescending(p => p.ParkingInfoId).ToListAsync();
                }
                else
                {
                    parkingInfos = await _context.ParkingInfos
                        ...
                        .Include(p => p.Pricing)
                        .Include(p => p.Slot)
                        .Include(p => p.Vehicle).Where(p => p.Status.Equals(status)).OrderByDescending(p => p.ParkingInfoId).ToListAsync();
                }
            }
            catch (Exception ex)
            {
        ...
        public async Task CheckActiveDate()
        {
            List<ParkingInfo> parkingInfos;
            var _context = new ParkingManagementContext();
            parkingInfos = await _context.ParkingInfos
                        ...
                        .Include(p => p.Pricing)
                        .Include(p => p.Slot)
                        .Include(p => p.Vehicle).Where(p => p.Status.Equals("Booked")).OrderByDescending(p => p.ParkingInfoId).ToListAsync();
            foreach(ParkingInfo p in parkingInfos)
            {
                if(p.CheckInTime.CompareTo(DateTime.Now) < 0)
                {
                    p.Status = "Active";
                    _context.ParkingInfos.Update(p);
                    await _context.SaveChangesAsync();
                }
            }
        }
